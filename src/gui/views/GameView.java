...
public class GameView extends View implements GameInterface {
  ...
  public void onResize() {
    ...
    this.map.setBounds(10, 30, mapPanelSize.width, mapPanelSize.height);
    this.map.revalidate();
    this.map.repaint();
    ...
    menuBar.setSize(w, 20);
    txtStats.setSize(sidebarWidth, 50 + 20 * game.getPlayers().size());
    dices.setSize(sidebarWidth, 50);
    int buttons = this.game.getGoal().getName().equals("Vertauschen") ? 4 : 3;
    scrollLog.setSize(sidebarWidth, h - txtStats.getHeight() - dices.getHeight() - 50 - BUTTON_SIZE.height * buttons);
    scrollLog.revalidate();
    scrollLog.repaint();
    
    joker.setSize(sidebarWidth, BUTTON_SIZE.height);
    button.setSize(sidebarWidth, BUTTON_SIZE.height);

    JComponent components[] = { txtStats, dices, scrollLog, button, joker, swap};
  ...
  protected void onInit() {
    ...
    this.joker = createButton("Joker");
    this.joker.addActionListener(new ActionListener() {

      @Override
      public void actionPerformed(ActionEvent e) {
        game.getCurrentPlayer().addTroops(5); 
        game.getCurrentPlayer().setJokerAvailable(false);
        joker.setEnabled(false);
        logLine("Joker eingesetzt. Es stehen nun 5 Truppen extra zur Verfügung.");
        map.repaint();
      }
    });
    ...
    JMenuItem home = new JMenuItem("Back to main menu");
  ...
  private void createSwapButton() {
    if (game.getGoal().getName() == "Vertauschen") {
      swap = createButton("Vertauschen");
      swap.setSize(sidebarWidth(), BUTTON_SIZE.height);
      this.add(swap);
      swap.addActionListener(new ActionListener() {
        @Override
        public void actionPerformed(ActionEvent e) {
          if (!game.allCastlesChosen()) {
            logLine("Es müssen alle Burgen ausgewählt worden sein.");
          }
          else if (newTurn) {
            logLine("Vertausche Burgen...");
            List<Castle> playerCastles = game.getCurrentPlayer().getCastles(game);
            List<Castle> otherCastles = game.getMap()
                                             .getCastles()
                                             .stream()
                                             .filter(c -> c.getOwner() != game.getCurrentPlayer())
                                             .collect(Collectors.toList());
            Castle castle1 = playerCastles.get((int)Math.random() * playerCastles.size());
            Castle castle2 = otherCastles.get((int)Math.random() * playerCastles.size());
            castle1.setOwner(castle2.getOwner());
            castle2.setOwner(game.getCurrentPlayer());
            map.repaint();
            newTurn = false;
          } else {
            logLine("Nur einmal pro Zug einsetzbar.");
          }
        }
  ...
  public void onNextTurn(Player currentPlayer, int troopsGot, boolean human) {
    ...
    /*
     * Joker is usable when the percentage of one's own castles from the number of 
     * all castles divided by the number of players is less than fourty percent 
     */
    
    double percentage = game.getCurrentPlayer().getCastles(game).size() 
        / (game.getMap().getCastles().size() / (double) game.getPlayers().size());
    if (game.getRound() != 1 && game.getCurrentPlayer().getJokerAvailable() && percentage <  0.4) {
