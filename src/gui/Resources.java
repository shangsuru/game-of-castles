...
public class Resources {
    ...
    private void saveScoreEntries() throws IOException {
      PrintWriter pw = new PrintWriter(new File("./res/highscores.txt"));
      scoreEntries.stream().forEach((entry) -> entry.write(pw));
      pw.flush();
      pw.close();
    }
    ...
    private void loadScoreEntries() {
      try (Stream<String> stream = Files.lines(Paths.get("./res/highscores.txt"))) {
        stream.forEach((line) -> addScoreEntry(ScoreEntry.read(line)));
      } catch (IOException e) {
        e.printStackTrace();
      }
    }
