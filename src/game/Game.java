...
public class Game {
  ...
  public void start(GameInterface gameInterface) {
    ...
    if(this.goal.getName().equals("Last Man Standing")) {
    	for(int i = 0; i < tempList.size(); i++) {
    		Player player = tempList.get(i);
    	...
    	playerQueue.add(tempList.get(0));
    }else {
  ...
  public void nextTurn() {
    ...
    // Blitzkriegmodus 
    if (round == 1 && allCastlesChosen() && goal.getName() == "Blitzkrieg") {
      // tries to find a castle with no enemies as neighbors and sets it as the capital
      Graph<Castle> graph = this.getMap().getGraph();
      boolean noCapital = true;
      for (Player player : this.getPlayers()) {
        List<Castle> castles = player.getCastles(this);
        for (Castle castle : castles) {
          if (graph.getNeighbors(graph.getNode(castle)).stream()
              .map(node -> node.getValue())
              .noneMatch(neighbor -> neighbor.getOwner() != player)) {
            capitals.add(castle);
            noCapital = false;
            break;
          }
        ...
        if (noCapital) {
          // there is no castle without enemies as neighbors, so we choose one randomly
          capitals.add(castles.get(0));
        }
    ...
    /**
     * Last Man standing troop distribution
     */
    int addTroops;
    if(round == 1) {
    	switch(this.goal.getName()) {
    	case "Blitzkrieg":
    		addTroops = GameConstants.CASTLES_AT_BEGINNING[1];
    		break;
    	case "Last Man Standing":
    		if(this.getCurrentPlayer() instanceof AI) {
    			addTroops = (int)(this.getMap().getCastles().size() * 0.6d);
    		}else {
    			addTroops = getRemainingCastles();
    		}
    		break;
    	default:
    		addTroops = GameConstants.CASTLES_AT_BEGINNING[0];
    	}
    }
      else {
        addTroops = this.goal.getName() == "Last Man Standing" ? (this.getCurrentPlayer() instanceof AI ? 4 : 3) : Math.max(3, numRegions / GameConstants.TROOPS_PER_ROUND_DIVISOR);
  ...
  public List<Castle> getCapitals() {
    return this.capitals;
  }
  
  /**
   * @return freeCastles number of castles with no owner
   */
  public int getRemainingCastles() {
	  int freeCastles = 0;
	  for(Castle castle : this.getMap().getCastles()) {
